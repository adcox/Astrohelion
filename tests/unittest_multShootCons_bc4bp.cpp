...
BOOST_AUTO_TEST_CASE(BC4BP_SEM_SOURCE_NODE){
	...
	Arcset_cr3bp seNodes = cr3bp_EM2SE(halfPlus, &seSys, semSys.getTheta0(), semSys.getPhi0(), semSys.getGamma());
	Arcset_bc4bp semNodes = bcr4bpr_SE2SEM(seNodes, &semSys, 0, 0);

	MultShootEngine corrector;
	Arcset_bc4bp correctedSet(&semSys);

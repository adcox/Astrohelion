...
void computeMap(vector<double> ICs, int numReturns, double C, adtk_cr3bp_sys_data sysData){
	int numICs = ICs.size()/6;
	adtk_cr3bp_sys_data tempSysData(sysData);

	printf("Beginning map creation for C = %.2f\n", C);
	printf("  Will integrate %d orbits, %d total integrations\n", numICs, 
		numICs*numReturns);

	adtk_simulation_engine engine(&tempSysData);
	engine.setVerbose(false);
	engine.addEvent(adtk_event::XZ_PLANE, 1, true);

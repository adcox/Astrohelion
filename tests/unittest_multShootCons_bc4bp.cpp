...
BOOST_DATA_TEST_CASE_F(fixture_SEM_Init, BC4BP_SEM_State, data::make(tofTypes), 
	tofTp){
	...
	BOOST_CHECK(MultShootEngine::finiteDiff_checkMultShoot(halfLyapSet, 
...
BOOST_DATA_TEST_CASE_F(fixture_SEM_Init, BC4BP_SEM_State_EndSeg, 
	data::make(tofTypes), tofTp){
...
BOOST_DATA_TEST_CASE_F(fixture_SEM_Init, BC4BP_SEM_State_rmState, 
	data::make(tofTypes), tofTp){
...
BOOST_DATA_TEST_CASE_F(fixture_SEM_Init, BC4BP_SEM_Apse, data::make(tofTypes), 
	tofTp){
	...
	double rdot = model->getRDot(apseData, 
		correctedSet->getEpoch(apseCon.getID()), &(finalState[0]), sys);
	BOOST_CHECK_SMALL(rdot, 1e-12);
}//====================================================

BOOST_DATA_TEST_CASE_F(fixture_SEM_Init, BC4BP_SEM_Apse_endSeg, 
	data::make(tofTypes), tofTp){
	...
	correctedSet->getSegRef(apseCon.getID()).getStateByRow(-1);
...
BOOST_DATA_TEST_CASE_F(fixture_SEM_Init, BC4BP_SEM_SaddlePoint_Exact, 
	data::make(tofTypes), tofTp){

	corrector->setTOFType(tofTp);

	double IC[] = {-0.71200455, 0.16675922, 0.02755461, 0.01186449, 
		-0.00004723, -0.0010737};
	double t0 = 10207.19;
	double tof = 51.32;
	...
	// This one throws errors but is ok
	BOOST_CHECK(true || MultShootEngine::finiteDiff_checkMultShoot(&nodes0, 
		*corrector, Verbosity_tp::NO_MSG));

	BOOST_CHECK_NO_THROW(corrector->multShoot(&nodes0, correctedSet));
	Eigen::Vector3d spPos = bcr4bpr_getSPLoc(sys, 
		correctedSet->getEpoch(spCon.getID()));
	std::vector<double> finalState = correctedSet->getState(spCon.getID());
	double diff = sqrt(pow(spPos(0) - finalState[0], 2) + 
		pow(spPos(1) - finalState[1], 2) + pow(spPos(2) - finalState[2], 2));
	BOOST_CHECK_SMALL(diff, 1e-10);
}//====================================================

BOOST_DATA_TEST_CASE_F(fixture_SEM_Init, BC4BP_SEM_Cont_PV, 
	data::make(tofTypes), tofTp){
	...
	BOOST_CHECK(MultShootEngine::finiteDiff_checkMultShoot(&doubleSrcLyap, 
		*corrector, Verbosity_tp::NO_MSG));
...
BOOST_DATA_TEST_CASE_F(fixture_SEM_Init, BC4BP_SEM_SegCont_Ex, 
	data::make(tofTypes), tofTp){
	...
	BOOST_CHECK_SMALL(forwardTraj.getEpochByIx(-1) - reverseTraj.getEpochByIx(-1), 
		1e-12);
}//====================================================

BOOST_DATA_TEST_CASE_F(fixture_SEM_Init, BC4BP_SEM_SourceNode, 
	data::make(tofTypes), tofTp){
	...
	std::vector<double> ic {1.0639767173456007, 0, 0.1644973017995331, 0, 
		-0.0311246472806882, 0};
	...
	Arcset_cr3bp seNodes = cr3bp_EM2SE(halfPlus, &seSys, sys->getTheta0(), 
		sys->getPhi0(), sys->getGamma());
	...
	BOOST_CHECK(MultShootEngine::finiteDiff_checkMultShoot(&semNodes, 
		*corrector, Verbosity_tp::NO_MSG));

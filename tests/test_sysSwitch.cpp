...
int main(void){
	...
	SimEngine engine;
	Arcset_cr3bp emHalo(&emSys);
	engine.runSim(haloIC, 2.77, &emHalo);
	emHalo.saveToMat("data/EM_Halo.mat");

	Arcset_cr3bp emNodes(&emSys, haloIC, 2.77, 10, Arcset::TIME);
	emNodes.saveToMat("data/EM_Nodes.mat");

	Arcset_cr3bp emHalo_inSE = cr3bp_EM2SE(emHalo, &seSys, 0.1, 0.2, 0.05);
	emHalo_inSE.saveToMat("data/EM_Halo_inSE.mat");

	Arcset_cr3bp emNodes_inSE = cr3bp_EM2SE(emNodes, &seSys, 0.1, 0.2, 0.05);
	emNodes_inSE.saveToMat("data/EM_Nodes_inSE.mat");

	Arcset_cr3bp eciNodes = cr3bp_rot2inert(emNodes, 0, 0);
	...
	engine.reset();
	Arcset_cr3bp seTraj(&seSys);
	engine.runSim(haloIC, 2.77, &seTraj);
	seTraj.saveToMat("data/SE_Traj.mat");

	Arcset_cr3bp seNodes(&seSys, haloIC, 2.77, 10, Arcset::ARCLENGTH);
	seNodes.saveToMat("data/SE_Nodes.mat");

	Arcset_cr3bp seTraj_inEM = cr3bp_SE2EM(seTraj, &emSys, 0.1, 0.2, 0.05);
	seTraj_inEM.saveToMat("data/SE_Traj_inEM.mat");

	Arcset_cr3bp seNodes_inEM = cr3bp_SE2EM(seNodes, &emSys, 0.1, 0.2, 0.05);
	...
	Arcset_bc4bp bcTraj = bcr4bpr_SE2SEM(seTraj, &bcSys, 0, 7.08);
	bcTraj.saveToMat("data/SEM_Traj.mat");

	Arcset_bc4bp bcNodes = bcr4bpr_SE2SEM(seNodes, &bcSys, 0, 7.08);

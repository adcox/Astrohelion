...
enum class Event_tp : int{
	...
	STATE_PLANE 	= 4,	/*!< occurs when a trajectory crosses a specific state value.
							 ...
							 * 	in the first <tt>param</tt> entry and the value of the variable in the
							 * 	second <tt>param</tt> entry. For example, if <tt>param</tt> = [3, -0.1],
	...
	CRASH			= 5,	/*!< occurs when trajectory falls below minimum acceptable
 	...
 	JC 				= 6, 	/*!< occurs when the Jacobi value reaches the specified value
		 					 ...
		 					 */
 	APSE 			= 7,	/*!< occurs when an apse is reached. The <tt>param</tt> array should have
		 					 ...
		 					 *	only periapses
		 					 */
 	DIST 			= 8, 	/*!< occurs when a distance from a primary is reached. The <tt>param</tt>
		 					 ...
		 					 */
	MASS 			= 9		/*!< occurs when the s/c mass reaches a minimum or maximum amount. The <tt>param</tt>

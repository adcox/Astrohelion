...
int main(void){
	...
	// Next, propagate to Moon apse (will be the close flyby)
	sim.clearEvents();
	sim.addEvent(apseCon);
	...
	BodyIx = 1;
	tpat_event apoEvent(&bcSys, tpat_event::APSE, 1, true, &BodyIx);
	sim.clearEvents();
	sim.addEvent(apoEvent);
	
	std::vector<double> q0 = arc2.getState(-1);
	double T0 = arc2.getTime(-1);
	tof = 90*24*3600/bcSys.getCharT();

	for(int i = 0; i < 20; i++){
		// Integrate to next apogee
		sim.runSim(q0, T0, tof);
		tpat_traj_bcr4bp tempTraj = sim.getBCR4BPR_Traj();
		tpat_nodeset_bcr4bp tempNodes(q0, &bcSys, T0, tempTraj.getTOF(), 6);
	...
	std::vector<double> flybyNode = capturedNodes.getState(0);
	flybyNode[3] = NAN;
	flybyNode[4] = NAN;
	flybyNode[5] = NAN;
	tpat_constraint fixFlybyPos(tpat_constraint::STATE, 0, flybyNode);

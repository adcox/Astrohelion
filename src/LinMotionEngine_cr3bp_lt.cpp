 ...
 *  @param pLaw pointer to a control law object; the control used on the linear 
 *  trajectory is stored in this object.
 *  @param numNodes How many nodes to create on the trajectory. Must be >= 2
 */
void LinMotionEngine_cr3bp_lt::getLinear(double eqPt[3], double f, double alpha,
	double x0[3], unsigned int motionTp, Arcset_cr3bp_lt *pArc, 
	ControlLaw_cr3bp_lt *pLaw, unsigned int numNodes){

	if(numNodes < 2){
		throw Exception("LinMotionEngine_cr3bp_lt::getLinear: "
			"Must specify at least 2 nodes");
	}

	const SysData_cr3bp_lt *pSys = 
		static_cast<const SysData_cr3bp_lt*>(pArc->getSysData());
	...
	A(2,0) = ddots[0];	A(2,1) = ddots[3];	// partial derivatives of pseudo-pot
	...
	if(eigensolver.info() != Eigen::Success){
		throw Exception("LinMotionEngine_cr3bp_lt::getLinear: "
			"Unable to compute eigenvalues of A matrix");
	}

	Eigen::VectorXcd vals = eigensolver.eigenvalues();
	std::vector<cdouble> eigVals = std::vector<cdouble>(vals.data(), 
		vals.data()+6);
	...
	for(unsigned int i = 0; i < 4; i++){
		if(verbosity >= Verbosity_tp::ALL_MSG){
			printf("Eigenvalue: %f %c %fj\n", real(eigVals[i]), 
				imag(eigVals[i]) < 0 ? '-' : '+', std::abs(imag(eigVals[i])));
		}
	...
	*pLaw = ControlLaw_cr3bp_lt(ControlLaw_cr3bp_lt::CONST_F_GENERAL, params);
	...
	const unsigned int full_dim = core_dim + ctrl_dim + 
		pow(core_dim + ctrl_dim, 2) + extra_dim;

	switch(motionTp){
		case LinMotion_tp::HYP:
			if(verbosity > Verbosity_tp::NO_MSG){
				printErr("LinMotionEngine_cr3bp_lt::getLinear: "
					"Hyperbolic mode is not implemented\n");
			}
			break;
		...
		case LinMotion_tp::UNSTAB_OSC:
		{
			if(numPureImag < 2){
				if(verbosity > Verbosity_tp::NO_MSG){
					printErr("LinMotionEngine_cr3bp_lt::getLinear: "
						"There is not a center subspace\n");
				}
			}else{
				...
				if(verbosity >= Verbosity_tp::ALL_MSG){
					printf("Choosing value = %f %c %fj\n", real(eigVals[ix]), 
						imag(eigVals[ix]) < 0 ? '-' : '+', 
						std::abs(imag(eigVals[ix])));
				}
				...
				double rate = std::abs(real(eigVals[ix]))*\
					(motionTp == LinMotion_tp::STAB_OSC ? -1 : 1);
				...
				if(motionTp == LinMotion_tp::OSC && std::abs(rate) > 1e-12){
					if(verbosity > Verbosity_tp::NO_MSG){
						printErr("LinMotionEngine_cr3bp_lt::getLinear: "
							"Requested oscillatory motion, but the oscillatory "
							"modes are spirals; aborting\n");
					}
					return;
				}
				if((motionTp == LinMotion_tp::STAB_OSC || 
					motionTp == LinMotion_tp::UNSTAB_OSC) && 
					std::abs(rate) < 1e-12){
					
					if(verbosity > Verbosity_tp::NO_MSG){
						printWarn("LinMotionEngine_cr3bp_lt::getLinear: "
							"No stable/unstable component to oscillatory "
							"motion; proceeding with pure oscillatory\n");
					}
				}
				...
				// Set alpha, beta (core_dim + 1) is always zero
				state[core_dim] = alpha;
				...
				for(t = 0; t < revs*period; t += t_step){
					for(unsigned int i = 0; i < 6; i++){
						...
						if(i != 2 && i < 5){
							state[i] = c3*exp(rate*t)*(u[ix]*cos(freq*t) - 
								w[ix]*sin(freq*t)) + 
								c4*exp(rate*t)*(w[ix]*cos(freq*t) + 
								u[ix]*sin(freq*t));
						}else{
							state[i] = 0;
						}
					...
					// Shift to be centered around equilibrium solution
					state[0] += eqPt[0];
					state[1] += eqPt[1];
					state[2] += eqPt[2];
					...
					if(t >= dt*nodeCount){
						...
						int ID = model->sim_addNode(node0, &(state.front()), t, 
							pArc, &eomParams, Event_tp::NONE);
						...
						seg.appendTime(t);
						model->sim_addSeg(seg, &(state.front()), t, pArc, 
							&eomParams);
						nodeCount++;
					}else{
						Segment &lastSeg = pArc->getSegRefByIx(-1);
				...
				int idf = model->sim_addNode(nodeF, &(state.front()), t, pArc, 
